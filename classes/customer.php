   ...
   class customer
   {
    ...
    public function insert_customers($data){
     ...
     public function login_customers($data){
      $email = mysqli_real_escape_string($this->db->link,$data['email']);
      return $password = mysqli_real_escape_string($this->db->link,$data['password']);
      if($email =""  || $password =""){
     ...
     public function customerLogin($data){
      ...
      if (empty($email) || empty($password)) {
         $msg = "<span class = 'error' >Email và mật khẩu không được để trống</span>";
         return $msg;
      }
      ...
      if( $result != false){
         ...
         $msg = "<span class = 'error'>Email hoặc mật khẩu không trùng khớp !</span>";
         return $msg;
     }
   }
   ...
   public function update_customers($data, $id){
      ...
      if($name  =="" || $zipcode =="" || $email =="" || $address =="" || $phone ==""){
       ...
       $query = "UPDATE tbl_customer SET name ='$name',zipcode='$zipcode',email='$email',address='$address',phone='$phone' WHERE id ='$id'";
         $result = $this->db->insert($query);
      if($result){
          $alert = "<span class = 'success'>Thay đổi hồ sơ thành công</span>";
          return $alert;
      } else{
           $alert = "<span class = 'error'>Thay đổi hồ sơ không thành công</span>";
          return $alert;
          }
      }
   }
